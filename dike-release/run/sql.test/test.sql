...
-- = [number of rows in the NEW-ORDER table for this table_district]
...
-- = [number of rows in the ORDER-LINE table for this table_district]
...
FROM (SELECT w.w_id, w.w_ytd, h.sum_h_amount
      FROM table_warehouse w,
           (SELECT h_w_id, SUM(h_amount) sum_h_amount FROM table_history GROUP BY h_w_id) h
...
-- Condition 10: 应该有问题
--Select c.c_w_id, c.c_d_id, c.c_id, c.c_balance from table_customer as c,
--   (Select ol.ol_w_id, ol.ol_d_id,ol.ol_o_id, sum(ol_amount) sum_ol_amount from table_order_line ol,
--   (Select o.o_w_id, o.o_d_id,o_c_id,o_id from table_oorder o,
--   (SELECT c_w_id, c_d_id,c_id, SUM(h_amount) sum_h_amount
--            FROM table_history
--            GROUP BY c_w_id, c_d_id,c_id) h where o.o_w_id = h.c_w_id and o.o_d_id = h.c_d_id
--            and o.o_c_id = h.c_id) as oc
--    where ol.ol_w_id = oc.o_w_id and ol.ol_d_id = oc.o_d_id and ol.o_o_id = oc.o_id
--    and ol.ol_delivery_d is not null group by ol.ol_w_id, ol.ol_d_id,ol.ol_o_id) as x
--
--    where