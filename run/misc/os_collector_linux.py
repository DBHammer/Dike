...
#   CPU usage and disk/memory IO.
#
#   This code is used in the OsCollector class. It is launched as
...
#
#   The command line arguments for this script are two variable
#   number of devices in the form "net_<devname>" "blk_<devname>"(both are necessary),
#   for example "net_eth0" or "blk_sda" for the first SCSI disk.
#
#   The output on stdout is one line for CPU info, followed by a
...
def main(argv):
    ...
    try:
        while True:
            ...
            # ----
            # Collect CPU, vmstat and memory information.
            # ----
            sysInfo = [elapsed, ]
    ...
    # ----
        ...
        if e.errno == errno.EPIPE:
            ...
            raise e
# ----
# Check if information in /proc meets our requirements 
# and get the init value of metrics(since some of them are incremental).
# ----
def initSystemUsage():
    global  procStatFD
    global  procVMStatFD
    ...
    procMemFD = open("/proc/meminfo", "r", buffering = 0)
    lastMemData = {}
    for line in procMemFD:
        line = line.split()
        if line[0] in ['MemTotal:', 'MemFree:', 'MemAvailable:', 'Buffers:', 'Cached:', ]:
            lastMemData[line[0]] = int(line[1])
    if len(lastMemData.keys()) != 5:
        raise Exception("not all elements found in /proc/meminfo")
    ...
    return [
            ...
            'vm_nr_dirty', 'mem_total', 'mem_free',
            'mem_avail', 'mem_buffer', 'mem_cache'
        ]
...
def getSystemUsage():
    ...
    for line in procMemFD:
        line = line.split()
        if line[0] in ['MemTotal:', 'MemFree:', 'MemAvailable:', 'Buffers:', 'Cached:', ]:
            newMemData[line[0]] = int(line[1])
    for key in ['MemTotal:', 'MemFree:', 'MemAvailable:', 'Buffers:', 'Cached:', ]:
        result.append(newMemData[key])
    return result

# ----
# Check if information in /sys/block/<dev> meets our requirements 
# and get the base value of metrics.
# ----
...
# ----
# Check if information in /sys/class/net meets our requirements 
# and get the base value of metrics.
# ----
