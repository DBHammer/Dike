 ...
 *          start each worker thread and coordinate between Terminal, Deamon, TxnCounter.
 *
...
public class Client {
    ...
    public void executeBenchmark() {
        ...
        if (runtimeProps.isDynamicLoad() || runtimeProps.isDynamicConflict() || runtimeProps.isDynamicTransaction()) {
            deamon = new Deamon(runtimeProps, this, txnCounter);
        ...
        for (int i = 0; i < terminalAlive; i++) {
            Terminal terminal = new Terminal(runtimeProps, connProps, this, i);
        ...
        if (deamonThread != null) {
            deamonThread.start();
            log.info("Start deamon thread for dynamic workload patterns");
        }
    ...
    public void signalTerminalEnd(int terminalID) {
        ...
        if (terminalAlive == 0) {
            ...
            // send stop signal to deamon thread
            if (deamon != null) {
                deamon.stop();
            }
            ...
            try {
                ...
                log.error("Get interruption while waiting for txn counter thread and deamon thread to finish "
                        + ie.getMessage());
            }
    ...
    // deamon write down conflict intensity information
    ...
    // deamon send fix conflict signal
    ...
    // deamon send unfix conflict signal
