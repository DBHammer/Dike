/*
 * Deamon - Dynamically change workload pattern and send control signals to Client.
 *
...
public class Deamon implements Runnable {
    ...
    private final Logger log = Logger.getLogger(Deamon.class);
    ...
    public Deamon(RuntimeProperty runtimeProps, Client client, TxnCounter txnCnt) {
        this.client = client;
        this.txnCnt = txnCnt;
    ...
    public void run() {
        while (!stopRunningSignal) {
            ...
            try {
                ...
                log.error("deamon thread catch interrupted exception while sleeping");
            }
        }
    } // end run
    ...
    // client send termination signal to deamon
    public void stop() {
